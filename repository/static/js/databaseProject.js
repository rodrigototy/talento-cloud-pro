import Projeto from "./classProject.js";

const databaseProject = [
  new Projeto(
    "Doação de Sangue",
    "doacao-sangue",
    "Projeto integrador de um site para doação de sangue"
  ),
  new Projeto("Aula 11", "aula-11", "Projeto da aula 11"),
  new Projeto("Aula 12", "aula-12", "Projeto da aula 12"),
  new Projeto("Aula 13", "aula-13", "Projeto da aula 13"),
  new Projeto("Aula 14", "aula-14", "Projeto da aula 14"),
  new Projeto(
    "Captura de Dados",
    "backend",
    "Criação de pagina HTML a partir de um array"
  ),
  new Projeto(
    "JavaScript I - Módulo 03",
    "js-1/mod-03",
    "Conexão de um arquivo HTML com JavaScript"
  ),
  new Projeto(
    "JavaScript I - Módulo 05",
    "js-1/mod-05",
    "Manipulação do DOM utilizando JavaScript"
  ),
  new Projeto(
    "JavaScript I - Módulo 06",
    "js-1/mod-06",
    "Catálogo de Produtos à Venda"
  ),
  new Projeto(
    "Tela de Login",
    "js-2/form-login",
    "Projeto de formulário de login"
  ),
  new Projeto(
    "Carrinho de Compras",
    "js-2/carrinho-compras",
    "Projeto carrinho de compras"
  ),
  new Projeto(
    "Eventos de Teclado",
    "js-2/eventos-de-teclado",
    "Capturar eventos do teclado e do mouse"
  ),
  new Projeto(
    "Validação de Formulários Proz",
    "js-2/validacao-formularios-proz",
    "Exemplo Proz de eventos que são usados para validar formulários"
  ),
  new Projeto(
    "Validação de Formulários Expert",
    "js-2/validacao-formularios",
    "Usando eventos para validar formulários"
  ),
  new Projeto(
    "Validação de CEP com BrasilAPI",
    "js-2/valida-cep",
    "Usando a aplicação BrasilAPI para validar CEP informado"
  ),
  //  new Projeto("", "", ""),
];

export default databaseProject;
