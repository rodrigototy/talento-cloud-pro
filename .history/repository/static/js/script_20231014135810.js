...
databaseProject.forEach((element) => {
  ...
  preItemProject.innerHTML = `<span>${element.index}</span><span>${element.nome}</span><span>${element.descricao}</span><span><a></a>${urlPageProject}${element.pasta}</span><span>${element.nome}</span>`;
